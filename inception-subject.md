...
7. [Prerequisites for validation](#7-prerequisites-for-validation)
8. [Bonus part](#8-bonus-part)
9. [Submission and peer-evaluation](#9-submission-and-peer-evaluation)

## 1. Preamble

> *Preamble image text:* “ONE CONTAINER IS NOT ENOUGH / WE NEED TO GO DEEPER.”
...
The latest tag is prohibited. No password must be present in your Dockerfiles. It is mandatory to use environment variables. Also, it is mandatory to use a .env file to store environment variables. It is strongly recommended that you use Docker secrets to store any confidential information. Any credentials, API keys, or passwords found in your Git repository (outside of properly configured secrets) will result in project failure. Your NGINX container must be the only entrypoint into your infrastructure via the port 443 only, using the TLSv1.2 or TLSv1.3 protocol.
...
*Diagram recreated from the PDF. Boxes represent Docker containers, cylinders represent volumes, dotted connections reproduce the source links, and numbers indicate ports.*
...
## 7. Prerequisites for validation
